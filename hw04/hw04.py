def filter(condition, lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    new_lst = [item for item in lst if condition(item)]
    lst.clear()
    lst.extend(new_lst)
...
def max_path_sum(t):
    ...
    def helper(node):
        ...
        left_sum = helper(node[1])
        right_sum = helper(node[2]) if len(node) > 2 else 0

        return node[0] + max(left_sum, right_sum)
...
def has_path(t, word):
    ...
    def helper(node, remaining_word):
        if not node:
            return False
        if node[0] == remaining_word[0]:
            if len(remaining_word) == 1:
                return True
            else:
                for child in node[1:]:
                    if helper(child, remaining_word[1:]):
                        return True
